...
In the end a Model was found with a RMSE of ``` {r final result } final_model_rmse <- RMSE(validation$rating, final_predict)``` running on the validation data set. 
...
summary(edx)
```
The dimensions of the data file are 
...
``` {r} length(problems)``` duplicate entries could be found.
