...
summary(edx)
```
The dimansions of the data file are 
```{r, echo=false} dim(edx)``` 
...
``` {r, echo=false} length(problems)``` duplicate entries could be found.
...
As you can see from the graph the minimum RMSE is reached with a lambda of ```{r, echo=false} lambda```
